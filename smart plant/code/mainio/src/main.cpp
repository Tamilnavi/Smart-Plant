...
<body>
  ...
  <div class="dashboard">
    ...
    <div class="sensor-grid" id="sensors">
        ...
        <div class="sensor-item" onclick="mistDevice('mist')">💧 Humidity:<br> <b>--%</b></div>
        <div class="sensor-item" onclick="pumpDevice('pump')">🌱 Soil Moisture:<br> <b>--</b></div>
  ...
  <script>
    ...
    if(data.aichat) {
      ...
      if (e.key === 'Enter') {
          sendMessage(); // Same function as your button click
      }});
      function pumpDevice(device) {
          fetch('/pumpp', {
              method: 'POST',
              headers: {
...
void initializeHardware() {
  ...
  controlRGB(255, 0, 0); // Initial red
...
void connectWiFi() {
  ...
  const char* ip_cstr = ipString.c_str();
  disp(ip_cstr);// print local ip
  Serial.println(ip_cstr);
  delay(4000);
}
// Web Server Setup
void initializeWebServer() {
  ...
  server.on("/pumpp", HTTP_POST, [](AsyncWebServerRequest *request){
    if(request->hasParam("pump", true)) {
      digitalWrite(PUMP_RELAY, !digitalRead(PUMP_RELAY));
    }
    request->send(200);
  });
  server.on("/mistt", HTTP_POST, [](AsyncWebServerRequest *request){
...
void handleAlerts() {
  ...
  if(state.pumpActive && millis() - state.pumpStart > 1000) {//pump off
    digitalWrite(PUMP_RELAY, LOW);
    state.pumpActive = false;
    state.pumpInt = false;
  }
  if(millis() - state.pumpStart > 2000){
    digitalWrite(PUMP_RELAY, LOW);
  }

  if(!state.pumpInt && millis() - state.pumpStart > 20000 ){// alert repeat 20sec
    state.pumpInt = true;
    digitalWrite(PUMP_RELAY, LOW);
  }
  //Humidity Alert
  if(state.humidity < 80 && !state.mistActive) {
  ...
  // mist off
  if(state.mistActive && millis() - state.mistStart > 20000) {
    digitalWrite(MIST_RELAY, LOW);
  ...
  if(!state.waterInt && millis() - state.waterStart > 60000){
    state.waterInt = true;
  }
  // weather temp 
  ...
  if(state.weatherDesc != state.report){//if any changes occur
...
// queryGroq function with this Groq implementation
String queryGroq(String prompt) {
  ...
  String systemMessage = "You are Smart Plant V1.0, a friendly and human kind AI responsive for plant care. ";
  systemMessage += "Always respond in a direct and related and friendly creative";
  systemMessage += "your are actually a Cactus plant.";
  systemMessage += "Created by Naveen, Anuub, and Bharath from Vels University. ";
  systemMessage += "Keep responses concise. ";
  ...
  systemMessage += "Rules: 1. Only answer the user's question directly 2. Never ask follow-up questions 3. Keep responses under 20 words with appropriate emojis, 4. Use language that user used.";
...
void handleAIProcessing() {
  ...
  if(internetAvailable && (millis() - lastAICall > 60000 || millis() < 10000)) {
    getWeatherData();
